...
type FormTemplateEmptyStateProps = {
  variant: ListEmptyVariant;
  onClearFilters?: () => void;
};

export function FormTemplateEmptyState({ variant, onClearFilters }: FormTemplateEmptyStateProps) {
  const { locale } = useLocale();
  const noData = pickLocalized(messages.formTemplate.empty.noData, locale);
  const filtered = pickLocalized(messages.formTemplate.empty.noFilteredResults, locale);
  const desc =
