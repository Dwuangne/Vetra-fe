...
export function TenantFilters({ keyword, onKeywordChange, onSearch, disabled, locale, className }: TenantFiltersProps) {
  const label = pickLocalized(messages.tenant.filters.keyword, locale);

  return (
    <div className={cn("flex flex-wrap items-end gap-3", className)}>
      <div className="flex min-w-[200px] max-w-md flex-1 flex-col gap-2">
        <label htmlFor="tenant-keyword" className="text-sm font-medium leading-none">
          {label}
        </label>
        <Input
          id="tenant-keyword"
