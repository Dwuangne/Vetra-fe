...
export function LocationFilters({ keyword, onKeywordChange, onSearch, disabled, locale, className }: LocationFiltersProps) {
  const label = pickLocalized(messages.location.filters.keyword, locale);

  return (
    <div className={cn("flex flex-wrap items-end gap-3", className)}>
      <div className="flex min-w-[200px] max-w-md flex-1 flex-col gap-2">
        <label htmlFor="location-keyword" className="text-sm font-medium leading-none">
          {label}
        </label>
        <Input
          id="location-keyword"
