...
type EventTimelineEmptyStateProps = {
...
export function EventTimelineEmptyState({
  ...
  const noData = pickLocalized(messages.event.empty.noData, locale);
  const filtered = pickLocalized(messages.event.empty.filtered, locale);
  const desc =
