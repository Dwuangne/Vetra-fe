...
type ProductInstanceEmptyStateProps = {
  variant: ListEmptyVariant;
  locale: Locale;
  onClearFilters?: () => void;
};

export function ProductInstanceEmptyState({ variant, locale, onClearFilters }: ProductInstanceEmptyStateProps) {
  const noData = pickLocalized(messages.productInstance.empty.noData, locale);
  const filtered = pickLocalized(messages.productInstance.empty.noFilteredResults, locale);
  const desc =
