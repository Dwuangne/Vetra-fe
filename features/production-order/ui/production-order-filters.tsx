...
type ProductionOrderFiltersProps = {
  keyword: string;
...
export function ProductionOrderFilters({
  ...
  const label = pickLocalized(messages.productionOrder.filters.keyword, locale);

  return (
    <div className={cn("flex flex-wrap items-end gap-3", className)}>
      <div className="flex min-w-[200px] max-w-md flex-1 flex-col gap-2">
        <label htmlFor="production-order-keyword" className="text-sm font-medium leading-none">
          {label}
        </label>
        <Input
          id="production-order-keyword"
          value={keyword}
